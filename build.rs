...
// fn run_configure(cwd: &Path) -> bool {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    if cfg!(feature = "generate-bindings") {
        ...
        // panic!("{}", build_parent_dir.to_str().unwrap());
        let _ = remove_dir_all(build_parent_dir.as_path());

        clone_repository(LIBINJECTION_URL, build_parent_dir.as_path())?;
        // run_autogen(build_parent_dir.as_path());
        // run_configure(build_parent_dir.as_path());
